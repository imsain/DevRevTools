// Generated from shared/lib/gitswap.mjs — do not edit this copy.
...
// run can happen after the change was already made.
//
...
// destroy it.
//
// Shared verbatim between plugins: rebuilding "before" without losing
// uncommitted work is the same problem whether what runs against the swapped
// file is a browser or a query.
 ...
 * command it would survive the swap and appear in the "before" state too —
...
export function createGitSwap({ tool, Err, stateDir, readJson, writeJson }) {
  function manifestPath(root) {
    return join(stateDir(root), 'swap-manifest.json');
  }

  /**
   * Swaps `files` to their contents at `ref`. Returns a restore function;
   * also leaves a manifest behind so `<tool> restore` can recover from a
   * crash.
   */
  function swapToRef(root, ref, files) {
    if (files.length === 0) {
      throw new Err(`Nothing differs from ${ref} — no "before" state to build`);
    }
    if (readJson(manifestPath(root))?.files?.length) {
      throw new Err(
        `A previous swap was not restored. Run "${tool} restore" first.`
      );
    }
    ...
    // Recorded before anything moves: `git checkout <ref> -- <file>`
    // overwrites the index too, so this is the only chance to learn what was
    // staged.
    const index = lines(
  ...
  function restoreSwap(root) {
    ...
    rmSync(join(stateDir(root), 'swap-backup'), {
      recursive: true,
      force: true
    });
  ...
  /**
   * Puts back the exact index entries the swap disturbed. Copying file
   * contents is not enough: `git checkout <ref> -- <file>` stages the ref's
   * blob, so a half-staged file would come back with the staged half silently
   * replaced. Rewriting only the recorded paths also leaves the rest of the
   * index alone.
   */
  function restoreIndex(root, manifest) {
    ...
    try {
      ...
      // An odd index state is not worth failing over — the working tree,
      // which holds the actual work, is already back.
    }
  }

  function hasPendingSwap(root) {
    return (readJson(manifestPath(root))?.files?.length ?? 0) > 0;
  }

  return { hasPendingSwap, restoreSwap, swapToRef };
}
