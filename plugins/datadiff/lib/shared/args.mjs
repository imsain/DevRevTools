// Generated from shared/lib/args.mjs — do not edit this copy.
 ...
 *
 * The two values Number() is too forgiving about are both rejected
 * explicitly: a bare `--settle` arrives as `true` and converts to a
 * plausible-looking 1ms, and an empty string converts to 0 — which is what
 * `--wait "$UNSET_VAR"` produces, the same silent skip by a different route.
 *
 * An absent flag returns `fallback`, so callers can write the default beside
 * the flag instead of `?? 4000` at every call site.
 */
export function createNumberFlag(Err) {
  return function numberFlag(name, value, fallback) {
    if (value === undefined) {
      return fallback;
    }
