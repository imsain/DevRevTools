...
That stripping fallback needs a Node that strips types by default: 22.18 or
23.6 and later. On an older one a TypeScript target says so and names both
ways out — upgrade Node, or install `typescript` in the target repo.
